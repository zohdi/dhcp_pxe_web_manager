...
HTTPS/TLS. The HTTPS variant uses the browser Web Crypto API (RSA-OAEP with
SHA-256). The browser encrypts password + a signed short-lived login challenge
with the public key; the server decrypts with the private key and validates the
challenge immediately before PAM/VAS auth.
"""
