...
- Local Manager (break-glass/full control)
- AD/VAS through Linux PAM, with app-managed ACL authorization
...
def current_identity():
    """Resolve the current session, revalidating AD ACL state every request."""
    ident = authz.resolve_session(session)
    if ident is None and session.get("auth_source") == "ad":
        session.clear()
    return ident
...
def _allowed_vlans_for_identity(ident):
    ...
    if ident.role == ROLE_VLAN_EDITOR and ident.auth_source == "ad":
...
def login():
    def render_ad_login():
        """Render a fresh signed, short-lived encrypted AD login challenge."""
        try:
            public_key_pem = get_public_key_pem(config.LOGIN_PUBLIC_KEY_PATH)
            login_challenge = create_login_challenge(config.FLASK_SECRET_KEY)
            crypto_ready = True
        except Exception as exc:
            logger.error(f"AD login crypto unavailable: {exc}")
            public_key_pem = ""
            login_challenge = ""
            crypto_ready = False
    ...
    if request.method == "POST":
        mode = (request.form.get("mode") or "ad").strip().lower()
        ...
        if mode == "ad":
            encrypted_credentials = request.form.get("encrypted_credentials") or ""

            # Fail closed before PAM unless the alias was explicitly approved.
            principal = acl_store.get_principal(username)
            if not principal or not principal["enabled"]:
                try:
                    acl_store.record_audit(username or "unknown", "ad", "AD_LOGIN", username, None, "DENIED", "No enabled ACL principal")
                except Exception:
                    pass
                flash("❌ AD login failed or this account is not authorized.", "danger")
                return render_ad_login()

            try:
                credentials = decrypt_credentials(
                    encrypted_credentials,
                    config.LOGIN_PRIVATE_KEY_PATH,
                )
                login_challenge = credentials.get("challenge", "")
                validate_login_challenge(
                    login_challenge,
                    config.FLASK_SECRET_KEY,
                    max_age=AD_LOGIN_CHALLENGE_MAX_AGE,
                )
                password = credentials.get("password", "")

                auth_result = ad_authenticator.authenticate(username, password)
                if auth_result.success:
                    session.clear()
                    session["role"] = principal["role"]
                    session["username"] = principal["alias"]
                    session["auth_source"] = "ad"
                    _audit("AD_LOGIN", principal["alias"], None, "SUCCESS", f"role={principal['role']}")
                    flash(f"✅ Logged in with AD as {principal['alias']}.", "success")
                    return redirect(url_for("index"))

                logger.warning(f"AD/PAM login failed for {username!r}: {auth_result.reason}")
                try:
                    acl_store.record_audit(username or "unknown", "ad", "AD_LOGIN", username, None, "DENIED", auth_result.reason)
                except Exception:
                    pass
                ...
                logger.warning(f"Encrypted AD login payload rejected for {username!r}: {type(exc).__name__}: {exc}")
                try:
                    acl_store.record_audit(username or "unknown", "ad", "AD_LOGIN", username, None, "DENIED", "Invalid encrypted credential payload")
                except Exception:
                    pass
                flash("❌ Secure AD login failed. Please reload the page and try again.", "danger")
                return render_ad_login()
            finally:
                try:
                    password = None
...
@app.route("/local-manager-login", methods=["GET", "POST"])
def local_manager_login():
    if request.method == "POST":
        username = (request.form.get("username") or "").strip()
        ...
        if username == config.MANAGER_USERNAME and check_manager_password(password):
            ...
            _audit("LOCAL_MANAGER_LOGIN", username, None, "SUCCESS")
            flash("✅ Logged in as DHCP Manager.", "success")
            return redirect(url_for("index"))

        try:
            acl_store.record_audit(username or "unknown", "local", "LOCAL_MANAGER_LOGIN", username, None, "DENIED", "Invalid local manager credentials")
        except Exception:
            pass
        flash("❌ Invalid manager username or password.", "danger")

    return render_template(
        "local_manager_login.html",
...
def access_control_edit_vlan(vlan_id: int):
    ...
    try:
        ...
        flash(
            ...
            f"({old_vlan['cidr']} → {updated['cidr']}). Existing AD assignments were preserved.",
            "success",
        )
...
def access_control_upsert_principal():
    ...
    try:
        ...
        flash(f"✅ Saved AD ACL for {principal['alias']} ({principal['role']}).", "success")
    except (ACLValidationError, ValueError) as exc:
...
def access_control_delete_principal(alias: str):
    ...
    flash(f"🗑️ Removed AD ACL for {alias}. Any active AD session is revoked on its next request.", "success")
    return redirect(url_for("access_control"))
...
def main():
    ...
    logger.info(f"AD/PAM service: {config.AD_PAM_SERVICE}; ACL database: {config.ACL_DB_PATH}")
