...
class AuthorizationService:
    ...
    def resolve_session(self, session_data: Mapping[str, object]) -> Optional[Identity]:
        ...
        # Built-in bcrypt Admin is independent of the ACL DB and always has full control.
        if source == "builtin" and role == ROLE_MANAGER:
            return Identity(username, ROLE_MANAGER, "builtin")

        # PAM users are continuously revalidated against the ACL DB so removal/role
        ...
        if source == "pam":
            ...
            if not principal or not principal["enabled"]:
                return None
            return Identity(principal["alias"], principal["role"], "pam")
    ...
    def can_view_ip(self, identity: Optional[Identity], ip: str) -> bool:
        ...
        if identity.role in (ROLE_MANAGER, ROLE_READONLY):
            return True
        if identity.role == ROLE_VLAN_EDITOR and identity.auth_source == "pam":
