...
def test_ad_login_payload_uses_https_web_crypto_not_plain_password():
    ...
    assert "crypto.subtle.importKey" in template
    assert "crypto.subtle.encrypt" in template
    assert "window.isSecureContext" in template
    assert "Secure AD login requires HTTPS" in template
    assert "vendor/rsa_oaep_sha256.js" not in template
    assert "HttpRsaOaep.encrypt" not in template
    assert 'passwordInput.value = ""' in template


def test_https_variant_does_not_bundle_http_rsa_helper():
    assert not (ROOT / "static" / "vendor" / "rsa_oaep_sha256.js").exists()
...
def test_https_apply_script_removes_stale_http_rsa_helper_without_copying_it():
    apply_script = source("apply_to_main.sh")
    files_block = apply_script.split("FILES=(", 1)[1].split(")", 1)[0]
    assert "static/vendor/rsa_oaep_sha256.js" not in files_block
    assert 'rm -f "$TARGET_DIR/static/vendor/rsa_oaep_sha256.js"' in apply_script


def test_readme_describes_https_web_crypto_requirement():
