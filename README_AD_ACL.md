# AD Authentication + VLAN ACL Overlay V1.4.1 HTTPS RSA (main branch)
...
For the HTTPS variant, the installer also removes a stale `static/vendor/rsa_oaep_sha256.js` if the checkout previously contained the HTTP variant; HTTPS uses native Web Crypto and does not need that helper.

## V1.3.4 HTTPS encrypted AD login prerequisites

The RSA login envelope uses Python `cryptography` on the server and the browser's native Web Crypto API (`crypto.subtle`) for RSA-OAEP/SHA-256. `requirements-auth.txt` installs `cryptography`.
...
**HTTPS is required for the encrypted AD form.** The login page uses the browser Web Crypto API (`crypto.subtle`), which requires a secure context. If the site is opened over ordinary LAN HTTP, AD submit is refused and the page tells the user to use HTTPS; it never falls back to plaintext submission. The submitted form contains ciphertext instead of the plaintext AD password.
