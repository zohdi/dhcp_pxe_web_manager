"""Application-managed AD ACL persistence for DHCP Manager.
...
class ACLStore:
    ...
    def normalize_alias(alias: str) -> str:
        ...
        if not value:
            raise ACLValidationError("AD alias is required")
        if len(value) > 128:
            raise ACLValidationError("AD alias is too long")
        if any(ch.isspace() for ch in value):
            raise ACLValidationError("AD alias cannot contain whitespace")
        return value
